...
function RouteComponent() {
  ...
  const methods = useForm<SystemConfig>({
    resolver: zodResolver(createSystemConfigFormSchema(m)) as unknown as Resolver<SystemConfig>,
  ...
  // 同步 settings 到 form（深合并默认值，保证新增字段如 auth.methods 始终有默认）
  useEffect(() => {
    if (settings) {
      reset({
        ...DEFAULT_CONFIG,
        ...settings,
        auth: { ...DEFAULT_CONFIG.auth, ...(settings.auth ?? {}) },
        email: { ...DEFAULT_CONFIG.email, ...settings.email },
        notification: {
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0 space-y-12 pt-2 lg:pt-0">
            ...
            <TabsContent value="auth" className="mt-0 space-y-10">
              <div className="space-y-2 pb-6 border-b border-border/30">
                <h2 className="text-2xl font-serif font-medium tracking-tight">
                  登录方式
                </h2>
